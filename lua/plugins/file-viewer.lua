local function toggle_file_explorer(dir)
        ...
        keys = {
            {
                "<leader>e",
                function()
                    toggle_file_explorer(vim.api.nvim_buf_get_name(0))
                end,
            ...
            },
            {
                "<leader>E",
                function()
                    toggle_file_explorer(vim.uv.cwd())
                end,
                ...
                callback = function()
                    ...
                    vim.keymap.set("n", "yc", function()
                        ...
                        if curr_entry then
                            ...
                            -- local cmd = string.format('powershell -command "Set-Clipboard -Value %s"', escaped_path)
                            -- local cmd = string.format(
                            --     'powershell -command "Get-Content -Path %s -AsByteStream | Set-Clipboard"',
                            --     escaped_path
                            -- )
                            -- local cmd = string.format(
                            --     'powershell -command "Get-Content -Path %s -Raw | Set-Clipboard"',
                            --     escaped_path
                            -- )
                            -- local cmd = string.format('powershell -command "Set-Clipboard -Path %s"', escaped_path)
                            --
                            -- local cmd = string.format("echo -n %s | wl-copy", escaped_path)
                            local cmd = string.format(
                                "wl-copy -t text/uri-list <<< file://%s",
                                vim.fn.fnameescape(escaped_path)
                            )
                            -- local cmd =
                    ...
                    vim.keymap.set("n", "yz", function()
                        ...
                        if curr_entry then
                            ...
                            -- )
                            -- local copy_cmd = string.format("echo -n %s | wl-copy", vim.fn.fnameescape(zip_path))
                            -- local copy_cmd = string.format("wl-copy < %s", vim.fn.fnameescape(zip_path))
                            local copy_cmd =
                                string.format("wl-copy -t text/uri-list <<< file://%s", vim.fn.fnameescape(zip_path))
                            -- wl-copy -t text/uri-list <<< "file://${whatever_i_want_to_copy}"
                            -- os.execute(string.format("wl-copy -t text/uri-list <<< 'file://%s'", vim.fn.fnameescape(zip_path)))
                            -- local copy_cmd = string.format(
            ...
            tell application "System Events"
              ...
              on error
                return "error"
                        ...
                        -- local output = vim.fn.system("powershell -command " .. vim.fn.shellescape(script)) -- Execute PowerShell command
                        local output = vim.fn.system("wl-paste") -- Use wl-paste to get clipboard content
                    ...
                    vim.keymap.set("n", "<M-i>", function()
                        ...
                        if curr_entry and curr_entry.fs_type == "file" then
                            ...
                            if vim.tbl_contains(supported_image_exts, ext) then
                                ...
                                local file_size_mb = tonumber(file_size_bytes)
                                        and tonumber(file_size_bytes) / (1024 * 1024)
                                    or 0
